...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    "mysql+mysqlconnector://root:root@db:3306/order"
)
...
class Order_Item(db.Model):
    ...
    order = db.relationship(
        "Order",
        primaryjoin="Order_Item.order_id == Order.order_id",
        backref="order_item",
    )
...
def get_all():
    ...
    if len(orderlist):
        return jsonify(
            {"code": 200, "data": {"orders": [order.json() for order in orderlist]}}
        )
...
def find_by_order_id(order_id):
    order = db.session.scalars(
        db.select(Order).filter_by(order_id=order_id).limit(1)
    ).first()
    if order:
        return jsonify({"code": 200, "data": order.json()})
    return (
        jsonify(
            {"code": 404, "data": {"order_id": order_id}, "message": "Order not found."}
        ),
        404,
    )
...
def create_order():
    ...
    for item in cart_item:
        order.order_item.append(
            Order_Item(book_id=item["book_id"], quantity=item["quantity"])
        )
    ...
    print(
        json.dumps(order.json(), default=str)
    )  # convert a JSON object to a string and print
    print()

    return jsonify({"code": 201, "data": order.json()}), 201
...
def update_order(order_id):
    try:
        ...
        if data["status"]:
            ...
            db.session.commit()
            return jsonify({"code": 200, "data": order.json()}), 200
    except Exception as e:
        return (
            jsonify(
                {
